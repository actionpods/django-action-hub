...
class Pod(BaseModel):
    ...
    def _get_title(self):
        return '%s\'s %s Pod' % (self.creator.username, self.focus.title)
...
#A clearly defined action that leads to an app
class Action(models.Model):
    title = models.CharField(max_length=100, unique=True)
    slug = models.SlugField(max_length=100, unique=True)
    pod = models.ForeignKey(Pod, blank=True)
    category = models.ForeignKey('ActionCategory', null=True)
    def __str__(self):
        return '%s' % self.title
...
class ActionCategory(models.Model):
    ...
    color = models.CharField(max_length=10, choices=COLORS, default='danger')
